...
gg <- ggplot(scores) + 
  ...
  geom_map(map=world_map, 
           aes(map_id=Country, fill=Population), color="black") + 
  expand_limits(x = world_map$long, y = world_map$lat) +  scale_fill_gradientn(colours=rev(magma(6)),
                                                                               name="Population",
                                                                               na.value = "grey100", 
                                                                               values =  rescale(seq(0, 400, length.out = 3)))
gg